namespace WinUpdExplorer
{
    internal static class XmlNamespaces
    {
        internal const string CoreUIService = "Microsoft-Windows-OneCore-CoreUI-Service";   
        internal const string Counters = "http://schemas.microsoft.com/win/2005/12/counters";
        internal const string Dsig = "http://www.w3.org/2000/09/xmldsig#";
        internal const string Empty = "";
        internal const string Events = "http://schemas.microsoft.com/win/2004/08/events";
        internal const string AssemblyV1 = "urn:schemas-microsoft-com:asm.v1";
        internal const string AssemblyV2 = "urn:schemas-microsoft-com:asm.v2";
        internal const string AssemblyV3 = "urn:schemas-microsoft-com:asm.v3";
        internal const string Task = "http://schemas.microsoft.com/windows/2004/02/mit/task";
        internal const string WindowsEvents = "http://manifests.microsoft.com/win/2004/08/windows/events";
        internal const string WininetConfiguration = "WinINet-Configuration";
        internal const string WMIConfigurationState = "http://schemas.microsoft.com/WMIConfig/2002/State";
        internal const string XsdSchema = "http://www.w3.org/2001/XMLSchema";
    }
}
